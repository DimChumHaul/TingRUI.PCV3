...
# **OrmLite**: 简单,高效,基于强类型的.NET Orm映射方案 

OrmLite: 一个轻量级.Net Orm映射方案 支持主流数据库
      [PostgresSQL、Sqlite3、MSSQL、Oracle、MySQL]
...
标注了 `.Core` 版本只包含 **.NET Standard 2.0** 运行时 只能在 ASP.NET Core 的版本上运行:
...
非官方版本的(OrmLite)插件由**社区**维护 插件开发社区:
...
### 步骤一：在代码中适配 OrmLiteConnectionFactory
...
## [类型转换工具](https://github.com/ServiceStack/ServiceStack.OrmLite/wiki/OrmLite-Type-Converters)
